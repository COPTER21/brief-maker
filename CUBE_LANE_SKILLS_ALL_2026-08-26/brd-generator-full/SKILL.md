...
description: >
  ...
  Input: RIF v2.1 + HTML จาก html-generator-v9 (แนะนำ) · Output: BRD .md + .docx + AI Review
...
RIF.md → html-generator-v9 → [ux gate + coverage gate ผ่าน]
   ↓ (RIF + HTML เข้าคู่กัน)
...
| **Lane Mode v2** ⭐ (2026-08-25) | ถูกเรียกจาก feature-lane-runner v2 S4 | **`01_PREBRIEF.md` + `01_FUNCTION_CHECKLIST.md` (FN-XX ledger) + `03_*_BRIEF.md` (declarations) + HTML v9 + LANE_BRIEF (LOCK/ASSUMED)** — RIF เป็น derived (requirement-intake-formatter PREBRIEF→RIF mode) ใช้เป็น §format เท่านั้น · ห้ามถาม · ทุก S-XX/FN-XX ต้องปรากฏใน BRD §Functions/§Scenario และ **§14.6 Functions Cut = หมวด "ไม่รองรับ" ของ FUNCTION_CHECKLIST** · Screen Inventory สกัดจาก HTML v9 (Pattern Q: list/wizard 5 steps/view tabs) |
...
| # | ไฟล์ | จากใคร | อ่านเพื่อ |
|---|---|---|---|
| 2 | `<feature>.html` | html-generator-v9 (ผ่าน ux+coverage gate แล้ว) | **หน้าจอ/flow ที่มีจริง** — Step 1.5 สกัด Screen Inventory · §5 Journey trace กับ route จริง · §8 status ที่มีจริงบนจอ |
...
Downstream ที่กระทบ:
  ...
  ✅ HTML v2 ถูกใช้เป็น source — ไม่ต้อง regenerate
     แต่อาจ run html-generator-v9 อีกครั้งเพื่อ "production cleanup"
...
**Layout/Pattern/CI spec จริง = หน้าที่ของ FRD** (frd-generator-v6 เป็น Design Authority, html-generator-v9 enforce) — BRD ห้ามระบุ Layout Template ID / pattern / สี / ขนาด
...
**สิ่งที่ BRD ไม่ทำแล้ว (ย้ายไป FRD):** Layout Template ID, Sync Read html-generator, CI lock check, Iron Rules compliance, Layout Deviation Log — ทั้งหมดเป็นหน้าที่ frd-generator-v6 + html-generator-v9
...
- **ห้าม** ระบุ Layout Template ID, pattern, สี, ขนาด, iron rules — เป็นหน้าที่ frd-generator-v6 (Design Authority) + html-generator-v9 (enforce)
