...
**FN-XX ใน HTML:** อ่านข้อความ FN เป็น "ประธาน+การกระทำ+ผลที่เช็คได้" แล้วเดินใน HTML จริง:
ทางเข้า (ปุ่ม/ช่อง) → handler มีจริง (ไม่ใช่ `onclick=""`/toast mock ล้วน) → ผลปรากฏ (สถานะ/ยอด/แถวเปลี่ยน) —
ครบ 3 ชั้น = ✓ · มีทางเข้าแต่ผลไม่ปรากฏ = △ partial · ไม่มีทางเข้า = ✗ · `keyword_hit_ratio` จาก script ใช้จัดลำดับดูก่อนเท่านั้น

**Declaration:** brief ต้องอยู่ในโฟลเดอร์ feature (`03_*_BRIEF.md`) — ไม่มีไฟล์ = ✗ แม้ HTML จะมี slot picker ·
เนื้อใน brief ต้อง trace S-XX/FN-XX (DOA: action ไหน · NTF: transition ไหน · CSQ: event→ท่อ · DOCCFG: doc_type) · brief ที่ประกาศท่อต้องห้าม (OC/DC-doc/SC) = ✗

**Pattern Q surface:** ใช้ `html-generator-v9/patterns/Q_transaction-document-archetype.md` เป็นรายชื่อ surface — ตรวจ "business ครบ" ไม่ตรวจ "โครงถูก" (นั่นคือ qc-ux Pass D) — ห้ามซ้ำงานกัน

**เมื่อ contract 2 ชุดขัดกัน (graph vs PREBRIEF):** PREBRIEF ใหม่กว่าแต่ graph คือเส้นข้าม feature — ไม่เลือกเอง: ลง `CONTRACT-CONFLICT` ให้ user แล้วตรวจต่อด้วยทั้งสองเกณฑ์ mark คนละแถว
