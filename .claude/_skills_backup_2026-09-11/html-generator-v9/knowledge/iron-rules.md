...
- **`.drawer-panel.wide`:** `1290px` — **ข้อยกเว้นเดียว: เอกสารที่ใช้ Pattern B2 (line editor)** เพราะ grid 8 คอลัมน์ + แถวขยายไม่พอที่ 920px — ประกาศเป็น class แยก ห้ามแก้ค่า base 920 เด็ดขาด (`patterns/B2_document-line-editor.md`)
...
### Rule #47.1 — Stepper Geometry Lock (v7.1 — จาก defect "stepper ไม่สมมาตร")
กฎบรรยายไม่พอ — เรขาคณิตต้องวัดได้ทุกข้อ:
1. **ทุก step กว้างเท่ากันเป๊ะ** (`flex:1 1 0` — ไม่ใช่ `flex:1` เฉย ๆ ที่ยอมให้ basis ต่างตาม content)
   · label ยาวไม่ดัน dot เคลื่อน — label `text-align:center` + truncate
2. **เส้น connector อยู่กึ่งกลางแนวตั้งของ dot เป๊ะ** (`top: calc(dot-size/2)`) และยาวชนกึ่งกลาง
   dot ซ้าย-ขวา — ห้ามเส้นลอยเหนือ/ใต้ dot แม้ 1px
3. dot ขนาดเดียวทั้งแถบ (page 28px / drawer `.stepper-circle` 32px) — is-current ห้ามขยาย dot
   (เน้นด้วยสี/น้ำหนัก ไม่ใช่ขนาด)
4. ระยะ dot→label คงที่ 8px ทุก step · แถบ stepper กึ่งกลาง container (`margin:auto`)
5. **CSS stepper = ลอกจาก skeleton verbatim** — ห้ามเขียน CSS stepper เองใน `#page-late`
   (ศักดิ์เดียวกับ master-combobox #94: kit คือ canonical)
- Render Gate: screenshot stepper แล้ววัด — dot ทุกตัวอยู่แนว baseline เดียวกัน + ช่องไฟระหว่าง
  dot เท่ากันทุกช่วง — เบี้ยว = FAIL
...
- [ ] **Rule #94 Master Combobox:** ทุก field ที่อ้าง master (product/customer/vendor/employee/dept/branch/BR/project/GL...) เป็น search combobox ตาม `master-combobox.js.txt` — state+handlers 6 ตัว+keyboard ↑↓/Enter/Esc ครบ ✓

---

## Group 18: Master-Backed Field (Rule 94) — v6.11 ⭐

### Rule #94 — Master-Backed Field = Search Combobox เสมอ (BREAKING)

**ทุก field ที่ค่าของมันมี master data รองรับ ต้อง render เป็น searchable combobox
pattern เดียวกันทั้งระบบ** — ห้ามเป็น `<select>` เปล่า, ห้ามเป็น text input เปล่า,
ห้ามออกแบบ dropdown เอง

ครอบคลุม (ไม่จำกัดแค่นี้ — ถามตัวเองว่า "ค่านี้มาจากตาราง master ไหม"):
- Product / Item / Service · Customer · Vendor / Supplier · Employee (ผู้ขอ/ผู้อนุมัติ/ผู้รับผิดชอบ)
- Department · Position · Branch / Warehouse / Location · Budget/BR · Project · Account/GL code
- ยกเว้น: **enum สั้นที่ไม่ใช่ master** (สถานะ, VAT mode, ความถี่) → `<select>`/radio ได้ ·
  **UoM ต่อสินค้า** → `<select>` scoped ใน line ได้ (list สั้น, ดู B2)

**Canonical source (verbatim):** `references/drawer-standard/master-combobox.js.txt`
(ตัวอย่าง = combobox "ผู้ขอ" จาก reference — BR / vendor / item ใน reference ใช้ contract เดียวกัน)

Contract ที่ห้ามเบี่ยง:
1. **State:** `{ query, open, highlight }` หนึ่งชุดต่อ combobox
2. **Handlers ครบชุด:** `onXSearchInput / onXSearchFocus / onXSearchKey / selectX / clearX` + suggest renderer
3. **Keyboard:** ↑↓ เลื่อน highlight · Enter เลือก · **Esc ปิด list ก่อน (stopPropagation) — drawer ยังเปิด** (Esc chain)
4. **Markup:** `.input` + icon search ซ้าย (padding-left 36px) + ขวา = ปุ่ม X (เมื่อเลือกแล้ว) หรือ chevron-down (ยังไม่เลือก) + suggest list absolute `max-height: 280px; overflow-y: auto`
5. **Item 2 บรรทัด** ตาม Rule #65 (avatar/initials | ชื่อหลัก 13px / บรรทัดรอง 12px truncate) + `<mark>` ไฮไลต์คำค้น
6. **Focus = เปิด list ทันที** ไม่ต้องพิมพ์ก่อน · **empty state ในเมนู** "ไม่พบ…" — เมนูห้ามหายเฉย ๆ
7. **ปิดเมื่อ:** เลือก / mousedown นอกกล่อง / Esc / blur — ครบ 4 (Rule #65 + #68)
8. **เลือกแล้ว** แสดง `CODE — ชื่อ` (เอกสาร/สินค้า) หรือชื่อเต็ม (บุคคล) + ปุ่ม X ล้างการเลือก
9. เมนูใน context ที่อาจชนขอบจอ (แถวล่างของตาราง) → drop-up ตาม Rule #66

การตรวจ (audit): ไล่ทุก `<select>` และ text input ในฟอร์ม — ถ้า field อ้าง master
แต่ไม่ใช่ combobox pattern นี้ = **FAIL Rule #94**

### Rule #94.1 — Cascade Clear (v7.1)
เลือกค่าจาก master combobox แล้วมี field อื่น autofill ตาม (เลือก vendor → ที่อยู่/เครดิตเทอม,
เลือก product → UoM/ราคา/ภาษี, เลือก employee → แผนก/ตำแหน่ง) — เมื่อกดปุ่ม X ล้างการเลือก:
1. **field ที่ autofill ตามมาต้องเคลียร์ด้วยทั้งหมด** — ห้ามค้างค่าเดิมของ record ที่ยกเลิกไปแล้ว
2. line items ที่ผูกกับค่านั้น (เช่น เปลี่ยน vendor แล้วรายการสินค้า scope ต่างกัน) →
   confirm ก่อนล้าง: "เปลี่ยน/ล้าง [X] จะล้างรายการที่เลือกไว้ N รายการ" (Pattern D)
3. clear แล้ว: query ว่าง + list เปิด + focus กลับ input (ตาม `clearX` ใน canonical source)
audit: ทุก `selectX` ที่เขียนค่าลง field อื่น ต้องมีคู่ `clearX` ที่ลบค่าเดียวกันครบ — ขาด = FAIL

---

## Group 19: Overlay Portal · List Full-Height · Responsive (Rules #95–#97) — v7.1

### Rule #95 — Overlay Portal (BREAKING — root cause "dropdown จม")
เมนู/dropdown/combobox list ที่ trigger อยู่ภายใน **scroll container** (`.table-wrap`,
drawer/modal body, B2 line grid, card ที่มี `overflow:auto|hidden`) —
z-index อย่างเดียวช่วยไม่ได้ เพราะโดน ancestor overflow **clip** ทิ้ง (stacking context ใหม่):
- ต้อง **portal ไป `#overlay-root`** ด้วย `portalMenu(trigger, menu)` จาก BASE-KIT
  (position:fixed คำนวณจาก getBoundingClientRect + flip-up + maxHeight ในตัว
  + reposition ตอน scroll/resize)
- trigger อยู่นอก scroll container (filter bar, page header) → `positionMenu` เดิม (#66) พอ
- เมนูที่ portal แล้วยังอยู่ใต้ระบบ `[data-overlay]` + global outside-click เดิม (#68) —
  ห้ามเขียน handler แยก
- audit: combobox ใดที่ ancestor มี `overflow` ไม่ใช่ `visible` แต่เมนูยัง absolute ใน DOM เดิม
  = **FAIL #95**

### Rule #96 — List Full-Height (ตารางยืดติดขอบล่าง)
หน้าที่เนื้อหลักคือ list/table:
- page wrapper ใส่ `.page-fill` → card ของตารางยืด `flex:1` → `.table-wrap` scroll ภายใน
  → pagination (`.table-foot`) ติดล่างของ card เสมอ — **ทุกขนาดจอ ขอบล่าง card ชิดขอบ viewport**
- thead sticky ใน `.table-wrap` (CSS มีใน skeleton แล้ว)
- ข้อมูลน้อย (ไม่เต็มจอ) → card ก็ยังยืดเต็ม, empty space อยู่ **ใน** ตาราง ไม่ใช่ใต้ card
- ห้ามให้ทั้ง page scroll เพราะตารางยาว — scroll อยู่ใน `.table-wrap` เท่านั้น
  (stat row + filter + header คงที่)

### Rule #97 — Responsive Desktop-Base (แทน doctrine desktop-only เดิม)
Base ออกแบบบน desktop (≥1180px) แต่ต้อง degrade อย่างมีระเบียบ:
| Tier | พฤติกรรม |
|---|---|
| ≥1180px | เต็มรูปแบบ — เหมือน v7.0 ทุกกฎ (#30 fluid ขาขึ้นคงเดิม) |
| 768–1180px | **adaptive:** sidebar → off-canvas (generic shell, มี `.nav-toggle` hamburger ใน shell-bar) หรือ icon rail (Pattern N) · drawer ทุกขนาด = `min(กว้างเดิม, 100vw)` · `.content` padding 16px · form grid 2 คอลัมน์ → 1 คอลัมน์ได้เมื่อ field แคบกว่า 280px (#17 เดิม) · ตารางกว้าง scroll ใน `.table-wrap` |
| <768px | ไม่ guarantee — `body min-width:768px` ให้ browser horizontal scroll (ไม่พังโครง) |
- **ห้ามซ่อน function ตอนจอแคบ** — ยุบเข้าเมนู/⋮ ได้ แต่ทุก action ต้องยังเข้าถึงได้
- B2 line editor: จอ <1180 ให้ grid scroll แนวนอนใน wrap ของตัวเอง — ห้ามยุบคอลัมน์ 8 ตัว
- Render Gate เพิ่ม viewport 1024px: screenshot แล้ว drawer ไม่ล้นจอ + sidebar ไม่ทับเนื้อหา + ไม่มี body horizontal scrollbar

---

## Group 20: Transaction Document Archetype (Rules #98–#101) — v9.0 ⭐

> ที่มา: มติ user 2026-08-25 หลัง craft `f-sales-order.html` — "งานเอกสาร เช่น SO ต้องมี pattern ประมาณนี้
> (landing = overview · เซ็น · doc preview · ไฟล์แนบ) หน้าสร้าง = stepper แบบนี้ ยิ่งหน้าตาราง+add สินค้าต้องเป็น
> layout แบบนั้นเป๊ะ ๆ — การกรอก item · ส่วนลด · VAT ต้อง fix ให้เหมือนกันเลย เผื่อไปทำ PR PO อื่น ๆ ในอนาคต"
> canonical: `references/document-archetype/_SOURCE_so-reference.html` · pattern: `patterns/Q_transaction-document-archetype.md`

### Rule #98 — Document Archetype Lock (BREAKING)
Feature ที่มี **เลขที่เอกสาร + สถานะเอกสาร + ลายเซ็น/อนุมัติ + PDF** = เอกสารธุรกรรม → ต้องใช้ Pattern Q ทั้ง 4 surface:
1. **List** — คอลัมน์ต้องมี เลขที่ (`.tbl-mono` primary, sortable) · สถานะเอกสาร (`docPill`) · **ลายเซ็น** (`renderSignProgress` = `n/N` + bar 46px) · ยอดรวม (right) · ⋮ menu — row click = view
2. **Create/Edit wizard** — drawer `.wide` 1290 · header eyebrow+h2+X · `.stepper` · footer contract (Rule #100)
3. **View drawer** — 920 · header (eyebrow · code `.tbl-mono` primary · `docPill` · summary line · action group ตามสถานะ + divider + copy/printer/download/X) · tabs (Rule #101) · footer "แก้ไขล่าสุด" + 🔒 เมื่อผ่านอนุมัติ
4. **PDF + Sign + Modals** — PDF tab `.a4` + 3 ช่องเซ็น · Sign tab `.tl` timeline + `empChip` · submit modal = DOA slot picker เลือก "คน" (avatar+position+name · มติ 2026-08-17) · reason modal สำหรับ ไม่อนุมัติ/ยกเลิก/พัก
- FRD/PREBRIEF วาดโครงต่างจากนี้ → **silent override** โครง/ลำดับ/ชื่อ ตาม Q (เนื้อหา business ยังตาม FRD) + log `Iron Rule Overrides Applied`
- ห้ามสร้าง "tab เอกสารแนบ" — เอกสารแนบเป็น section ใน tab รายละเอียด (landing)
- audit: `doc_archetype_*` ใน `scripts/audit.sh` (ทริกเกอร์เมื่อไฟล์มี `vat_mode` หรือ `renderSignTab`)

### Rule #99 — Line Editor v2 Lock (BREAKING — แทน canonical PR ของ v8)
ทุกเอกสารที่มี line items (PR/PO/QT/SO/GRN/INV/CN/DN/RTV) ลอก `references/document-archetype/line-editor-v2.js.txt` verbatim:
- grid `.tbl.line-tbl` 9 ช่อง widths **26 / — / 64 / 92 / 92 / 78 / 72 / 104 / 54** (# · สินค้า · จำนวน · หน่วย · ราคา/หน่วย · ส่วนลด % · ภาษี · จำนวนเงิน · actions) — ห้ามเพิ่ม/ลด/สลับ/เปลี่ยนกว้าง
- item = combobox `.item-combo` (Rule #94) แสดง `CODE — ชื่อ` · หน่วย = `<select>` เมื่อ `units.length>1` · ราคา disabled เมื่อ `!CFG.allowPriceOverride` · override → border warning
- ภาษี = `taxBadgeV` pill (ไม่คิด / VAT n% / NET n%) คลิกไม่ได้ — แก้ในแถวขยาย
- แถวขยาย = **lean 1-line**: VAT segmented [ไม่คิด|บวกเพิ่ม|รวมแล้ว (NET)] + % + `= ฿vat` │ หมายเหตุ │ [ระบบเสนอ ฿ + ใช้ราคาระบบ] │ chevron-up — ห้ามมีแถวที่ 2
- `calcLineVat` (discount % หรือ ฿ cap subtotal → net → vat 3 โหมด) · `migrateLine` · `totals` (before/vat/after/ebAmt/coupon/grand) — **ห้ามคำนวณที่อื่น**
- ส่วนลดท้ายบิล = card toggle → segmented [จำนวนเงิน (฿)|เปอร์เซ็นต์ (%)] + input 180px suffix · pill "อัตโนมัติ · src" เมื่อมาจากข้อตกลง/โปรฯ
- `renderLineSummary` ลำดับ: [hard-warn] → ราคาก่อน VAT → VAT (purple) → ยอดรวมหลัง VAT → [WHT] → [ส่วนลดท้ายบิล] → [คูปอง] → ยอดสุทธิทั้งหมด (16px primary) → [hook business]
- `updateLine` ต้องคืน focus+caret หลัง render · `free-row` ของแถม แก้/ลบไม่ได้
- optional blocks (ตัดได้ตาม business): `stockAlert` · `pricingPanel` · WHT · ปุ่ม `คำนวณราคาใหม่` — ตัดแล้ว block ที่เหลือต้องคงลำดับ
- ขยาย field เฉพาะบรรทัด (lot/expiry/serial) → ต่อในแถวขยายเดียวกันด้วย divider หรือ modal — ห้ามเพิ่มคอลัมน์ใน grid

### Rule #100 — Wizard Step Contract
เอกสารธุรกรรมใช้ 5 steps ชื่อ+ลำดับล็อก: **① เลือกแหล่งที่มา · ② ข้อมูลหลัก[เอกสาร] · ③ รายการสินค้า · ④ เอกสารแนบ · ⑤ ตรวจสอบและยืนยัน**
- ไม่มี line items → ตัด ③ (เหลือ 4) · ไม่มีเอกสารต้นทาง → ① เหลือ tile เดียว "สร้างใหม่" (ยังคง step ไว้เพื่อเลือก template/ประเภท) หรือตัดได้ถ้าไม่มีทางเลือกเลย — ห้ามสลับลำดับ ห้ามเปลี่ยนชื่อ
- ทุก step เปิดด้วย `STEPH(icon, title, hint)` · step ① = tile buttons 2 ช่อง + card ค้นหาเอกสารต้นทาง (Enter เลือก) · step ④ = `.upload-zone` · step ⑤ = review read-only (`<dl>` grid + `linesTableView`) + เหตุผล override ถ้ามี
- footer: ซ้าย [ย้อนกลับ] secondary (step>1) · ขวา [ยกเลิก] ghost → [ถัดไป] primary / step สุดท้าย [บันทึกแบบร่าง] secondary → [บันทึกและส่งอนุมัติ] primary (disabled เมื่อ hard control)
- `wizardNext()` validate ต่อ step + `markErr` + toast ตาม microcopy · stepper done คลิกย้อนได้ · edit mode เริ่ม step ②

### Rule #101 — View Tab Contract
tabs ของ view drawer เอกสาร: **รายละเอียด › [domain tabs ≤ 2] › PDF Preview › ลายเซ็น / อนุมัติ › ประวัติ** (`tabBtn(key,label,icon)`: detail/file-text · pdf/file · sign/pen-tool · history/history)
- **รายละเอียด (landing)** ลำดับ section: banners → ข้อมูลเอกสาร (KV 2 col) → สถานะ (3 col pill + `pgl`) → เอกสารอ้างอิง → คู่ค้าและผู้รับผิดชอบ → เงื่อนไข → ยอดรวม → รายการสินค้า (`linesTableView`) → **เอกสารแนบ (N ไฟล์ · Document Center)** → ข้อมูลระบบ — ใช้ `SECWRAP(SEC(icon,title)+…)` เท่านั้น
- domain tabs (SO: สต๊อกและการส่ง · เครดิต·บิล·ชำระ / PO: รับของ · วางบิล / GRN: QC…) แทรก**หลัง** รายละเอียด เท่านั้น
- PDF Preview = toolbar (ตัวอย่าง PDF · A4 · n/n + ดาวน์โหลด/พิมพ์ ghost sm) + `.a4` ตาม `thai-doc-pdf-generator` + 3 ช่องเซ็น (ผู้จัดทำ · ผู้อนุมัติ · [คู่ค้า]ยืนยัน)
- ลายเซ็น / อนุมัติ = card KV 4 col (DOA entry `.kbd` · สถานะ+ขั้น n/N · ผู้ส่ง · ยืนยันโดย) + progress → `.tl` timeline ขั้น 1..N พร้อม `empChip` + ปุ่ม อนุมัติ/ไม่อนุมัติ inline ที่ขั้นปัจจุบัน (canAct) → ผลลัพธ์ → ประวัติอนุมัติ
- ประวัติ = `.tl` จาก `audit[]` append-only (ok/warn/bad)

---

## Group 21: Combobox Anatomy · Lean List · Stable Screen (Rules #102–#103) — v9.0 ⭐

> ที่มา: feedback user 2026-08-25 — "อันไหนเป็น search dropdown ได้ให้ทำ · ลักษณะต้องคล้ายกัน คนต้องมี icon คน ชื่อ ตำแหน่ง
> เรียงถูกต้อง item ก็ด้วย · หน้าจอนิ่งไม่เบี้ยว · ข้อมูล lean · list view อย่าซ้อนข้อมูล คนละตัวแยกคอลัมน์"

### Rule #102 — Search Dropdown Everywhere + Option Anatomy Lock (BREAKING)
**A. เมื่อไหร่ต้องเป็น search combobox** — field ที่ค่ามาจาก "รายการ" ใด ๆ: คน (พนักงาน/ผู้อนุมัติ/ผู้รับผิดชอบ) · สินค้า/บริการ ·
ลูกค้า/ผู้ขาย/คู่ค้า · แผนก/ตำแหน่ง/สาขา/คลัง · โครงการ/งบ/GL · เอกสารต้นทาง (QT/PR/PO) · template/ประเภท ที่ >7 ค่า
→ **ต้องเป็น search combobox** (canonical: `combo()` ใน `doc-create-wizard.js.txt` หรือ `master-combobox.js.txt`) — `<select>` ใช้ได้เฉพาะ enum ตายตัว ≤7 ค่า (สถานะ, VAT mode, ช่องทาง 3-4 ตัว)
- พิมพ์แล้วกรองทั้ง code+ชื่อ · focus เปิดทันที · ↑↓/Enter/Esc · portal ลอย (#95) · flip-up (#66) · "ไม่พบรายการ" state · clear = cascade (#94.1)

**B. Option anatomy — ทุก dropdown ในไฟล์ต้องเรียงแบบเดียวกัน** (`.combo-pop button` = flex · gap 10 · padding 8/12):
| ประเภท | ซ้าย (icon) | บรรทัด 1 (12.5px/600 navy) | บรรทัด 2 (11px mute) | ขวา (10.5px soft, optional) |
|---|---|---|---|---|
| **คน** | `.emp-av` initials 26px (หรือ `user` icon ถ้าไม่มีชื่อย่อ) | **ชื่อ** | **ตำแหน่ง · แผนก** (· รหัส) | — |
| **สินค้า/บริการ** | `package` icon | `code · หมวด` (`.tbl-mono` mute) | **ชื่อสินค้า** (600 navy) | `฿ราคา/หน่วย` + `ATP n` / `บริการ` |
| **ลูกค้า/คู่ค้า/องค์กร** | `building-2` icon | **ชื่อ** | `code · กลุ่ม · เครดิต/เงื่อนไข` | — |
| **เอกสาร (QT/PR/PO)** | `file-text` icon | **เลขที่** (`.tbl-mono` primary) | คู่ค้า · วันที่ · ยอด | pill สถานะ |
| **สถานที่/คลัง/แผนก** | icon ประเภท (`warehouse`/`map-pin`/`building`) | **ชื่อ** | code · ผู้ดูแล | — |
- **ลำดับห้ามสลับ**: คน = icon→ชื่อ→ตำแหน่ง (ไม่ใช่ตำแหน่งก่อนชื่อ) · สินค้า = code บนชื่อ (code หาเร็วกว่า)
- ค่าที่เลือกแล้วในช่อง: คน = ชื่อ (มี `empChip` เมื่อแสดงในตาราง/DOA slot) · สินค้า = `CODE — ชื่อ` · เอกสาร = เลขที่
- highlight `.hi` = bg primary-08 · ทุก option `onmousedown=preventDefault` กัน blur ก่อน click
- audit: `combo_anatomy` — option ที่มี `emp-av`/`user` icon ต้องตามด้วย 2 บรรทัด · `<select>` ที่มี >7 `<option>` = WARN

### Rule #103 — Lean Data + List Column Atomicity (ย้ำ #40) + Stable Screen (ย้ำ #35/#95) (BREAKING)
**A. List view ห้ามซ้อนข้อมูล**
- **1 ข้อมูล = 1 คอลัมน์** — สถานะเอกสาร / สถานะจัดส่ง / สถานะจ่าย / ลายเซ็น / ยอดรวม / วันที่ / ผู้รับผิดชอบ = คนละคอลัมน์ ห้ามยัด pill 2-3 อันใน cell เดียว ห้ามเอาวันที่ไปต่อท้ายชื่อ
- บรรทัดรองใน cell ใช้ได้เฉพาะ **meta ของข้อมูลเดียวกัน** (code ใต้ชื่อลูกค้า · หมวดใต้ชื่อสินค้า) — ไม่ใช่ข้อมูลอีกตัว
- คอลัมน์ >8 → ย้ายที่สำคัญน้อยไป drawer/⋮ ไม่ใช่ยุบรวม (#16) · แกนสถานะหลายแกน (SO: เอกสาร/จัดส่ง/จ่าย/สต๊อก/ลายเซ็น) = แต่ละแกนคอลัมน์ตัวเอง ตัวอักษรกลาง
**B. ข้อมูล lean**
- แสดงเฉพาะสิ่งที่ผู้ใช้ใช้ตัดสินใจ ณ จุดนั้น · ข้อมูลรอง → `title` (hover) / บรรทัดรอง / drawer · ไม่มีคำอธิบายยาว ไม่มี hint เปลือย (#67.1) ไม่มีศัพท์ภายใน (#81)
- card/summary ใช้ `KV` (label 11px soft + value 600) ไม่เขียนเป็นย่อหน้า · ตัวเลขเงิน `tabular-nums` เสมอ
**C. หน้าจอนิ่ง ไม่เบี้ยวไปมา**
- ทุก overlay (dropdown/popover/menu/tooltip) **ลอยทับ** ผ่าน portal `position:fixed` — ห้ามดันเนื้อหาใต้มัน (#35, #95)
...
### Rule #104 — One Feature · One Menu (BREAKING · 2026-08-25)
> ที่มา: feedback user หลัง dry-run Consent PDPA — HTML สร้างเมนูซ้าย 2 อัน ("สิทธิ์การติดต่อ" + "ตรวจสิทธิ์ติดต่อ") ทั้งที่เป็น feature เดียว

- **1 feature = 1 รายการใน sidebar** (ชื่อ feature ตาม catalog) · route หลัก 1 ตัว `#/<feature>`
- ส่วนย่อยของ feature (หน้าตรวจ/ค้นหา · ตั้งค่า · รายงาน/สรุป · ประวัติ · import) → **`.tabs` ใต้ page header** (`#/<feature>/<tab>` refresh-safe) หรือ segmented control · action ที่ไม่ใช่หน้าเต็ม → ปุ่มบน `.ph-actions` เปิด drawer/modal
- เมนูอื่นใน sidebar = feature ข้างเคียงของ module ตาม catalog (context เท่านั้น · คลิกได้ = placeholder "อยู่นอก scope รอบนี้") — ห้ามสร้างเมนูที่ FRD/catalog ไม่มี
- ข้อยกเว้นเดียว: PREBRIEF/LANE_BRIEF ระบุชัดว่า scope รอบนี้ครอบ **หลาย feature** (module HTML) — แต่ละ feature ก็ยังได้เมนูละ 1
- ตัวอย่างถูก: Consent PDPA = เมนู "สิทธิ์การติดต่อ" → tabs: ทะเบียน · ตรวจสิทธิ์ · ประวัติ · ตั้งค่าวัตถุประสงค์
- audit: sidebar link ที่ href แชร์ segment แรกเดียวกัน (`#/consent`, `#/consent/check`) >1 รายการ = WARN #104 (qc-ux ตัดสิน BLOCK ถ้าเป็น feature เดียวกันจริง)


### Rule #105 — Demo / Persona Controls (2026-08-25)
> ที่มา: Consent PDPA run 2 — segmented "Officer (ปิดบัง) / DPO (เปิดเผย)" ถูกวางใน page header ข้างปุ่มหลัก ดูเหมือน feature จริง

- ตัวสลับบทบาท/สิทธิ์/ข้อมูลตัวอย่าง เพื่อ demo (persona switch · masking preview · seed switch · "ดูในฐานะ…") = **ของ prototype** — ระบบจริงได้บทบาทจาก login/Policy Center
- ต้องอยู่ใน **`.demo-strip`** (แถบเล็ก fixed ล่างซ้ายที่มี "Prototype · v1.0.0") หรือ topbar ข้าง avatar ผู้ใช้ — ห้ามอยู่ใน `.ph-right`/`.ph-actions`/page header
- ใน `.demo-strip` ใช้ label ชัด "Demo: บทบาท [Officer|DPO]" · ไม่ใช้ `.seg`/`.tabs` แบบเดียวกับ UI จริง (กันสับสน)
- page header actions = business action เท่านั้น (สร้าง/นำเข้า/ส่งออก/ตัวกรองหลัก)
- audit: `.ph-right`/`.ph-actions` มี `persona|demo|role-switch` หรือข้อความ "(ปิดบัง)/(เปิดเผย)" = FAIL #105 · render gate: `DEMO_CONTROL_IN_HEADER` = BLOCK

### Rule #40.1 — List Row Geometry (ย้ำ #40/#103 เชิงตัวเลข · 2026-08-25)
- แถว list สูง **≤ 52px** (2 บรรทัด ชื่อ+meta) · เกิน 64px = BLOCK · avatar ต้องอยู่**ข้างซ้าย**ชื่อผ่าน `.tbl-person` (inline-flex) ห้าม block ซ้อนบน
- filter bar = `.filter-row` แถวเดียว (search 280–320 + select ≤200 ต่อตัว · เกินก็ wrap เป็น 2 แถวได้ แต่ห้าม select กว้างเต็มบรรทัด) · สูง ≤ 56px
- ข้อมูลเดียวกันห้ามแสดง 2 คอลัมน์ (เช่น "สถานะรวม" กับ "สรุปสิทธิ์" ที่นับจากสิ่งเดียวกัน) — เลือกอันเดียว อีกอันไป drawer
- นับจำนวนที่ page header กับ footer ต้องมาจากตัวแปรเดียว (`N รายการ`) — ไม่ตรง = BLOCK
