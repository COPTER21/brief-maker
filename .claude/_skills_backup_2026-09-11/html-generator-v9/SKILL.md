...
description: >
  ...
  Iron Rules #98–#105 (doc archetype, combobox anatomy คน/สินค้า, lean list ไม่ซ้อนคอลัมน์, layout นิ่ง, 1 feature = 1 เมนู).
...
| **1 feature = 1 เมนูซ้าย** | feature หนึ่งมีรายการใน sidebar **รายการเดียว** — หน้าจอย่อย (ตรวจสิทธิ์ · ตั้งค่า · รายงาน · ประวัติ) เป็น **tab / segmented ภายในหน้า** หรือปุ่มบน page header / drawer · ห้ามแตกเป็นเมนูที่ 2 · เมนูอื่นใน sidebar = feature ข้างเคียงของ module (mock ไว้เป็น context เท่านั้น ไม่ทำ) | #104 |
| **แถว list เตี้ย · filter แถวเดียว · demo ไม่อยู่ header** | แถว ≤52px (avatar ซ้ายชื่อด้วย `.tbl-person`) · `.filter-row` แถวเดียว · count header=footer ตัวแปรเดียว · persona/demo switch อยู่ `.demo-strip` เท่านั้น | #40.1, #105 |
...
27. **Demo controls (Iron Rule #105)** — persona/masking/seed switch อยู่ `.demo-strip` เท่านั้น · **#40.1** แถว list ≤52px · filter-row แถวเดียว · ข้อมูลเดียวกันไม่ 2 คอลัมน์
26. **One Feature · One Menu (Iron Rule #104)** — sidebar มีรายการของ feature นี้ 1 รายการ · หลายส่วน = tabs ในหน้า (`.tabs` ใต้ page header) หรือ segmented · ไม่ใช่เมนูซ้ายหลายอัน
...
| `knowledge/iron-rules.md` | Iron Rules consolidated (#1–#49 + #94–#105) |
