...
9. **Demo/Persona controls (#105)** — ตัวสลับบทบาท/ข้อมูล demo (เช่น "Officer (ปิดบัง) / DPO (เปิดเผย)") ห้ามอยู่ใน page header/`.ph-right` — ต้องอยู่ใน demo strip ล่างซ้าย (`.demo-strip`) หรือ topbar ข้าง avatar · page header actions = business action เท่านั้น
8. **One Feature · One Menu (#104)** — sidebar มีรายการของ feature นี้ >1 (เช่น "สิทธิ์การติดต่อ" + "ตรวจสิทธิ์ติดต่อ") = BLOCK · ส่วนย่อยต้องเป็น `.tabs` ใต้ page header / segmented / drawer · เมนูอื่นต้องตรง catalog ของ module
...
3.2b **Geometry Gate (อัตโนมัติจาก `render_shots.py` v3 → `RENDER.json` + บรรทัด `GEOMETRY_BLOCK`):** แถว list สูง >64px = BLOCK #40/#103 · filter bar สูง >60px (ซ้อนเป็นหลายแถว) = BLOCK · avatar อยู่เหนือชื่อใน cell (ไม่ใช่ `.tbl-person` inline) = BLOCK #40 · persona/demo control ใน `.ph-right` = BLOCK #105 · body h-scroll = BLOCK #97 · `pageerrors` >0 = BLOCK — คัดลอกทุก GEOMETRY_BLOCK ลง report พร้อมวิธีแก้ (ใช้ kit class ที่ถูก)
