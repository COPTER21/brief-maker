...
description: >
  ...
  "ตัดสิน layout" เป็น "สกัด+ตรวจ+บันทึก pattern จากหน้าจอจริง" + Sync Read → html-generator-v6
  ...
  Input: BRD v2.2 APPROVED + HTML จาก html-generator-v6 (แนะนำ) + Security Bible
...
> - ⭐ Sync Read ย้าย → **html-generator-v6** (v4 ไม่มีในเครื่องแล้ว) + fallback หา v* สูงสุด
...
> **Changelog v6.0 (2026-07-06) — Design Authority Edition** *(การอ้าง v4 ด้านล่างเป็นบริบท ณ ตอนนั้น — ปัจจุบัน superseded โดย v6.1 ให้ยึด html-generator-v6)*:
> - ⭐ **FRD = Design Authority** — Phase 1.5 ใหม่: รับ Screen Inventory หยาบ + UI Signals จาก BRD v2.1 §14.6 → ตัดสิน pattern A-M ของ html-generator-v4 เอง + Layout Decision Log ใน 01_UI §1.0 (บทบาทย้ายมาจาก BRD ตั้งแต่ 2026-07-06)
> - ⭐ **Sync Read → html-generator-v4** — pattern A-M ความหมายใหม่, Drawer v2 920/680, Sidebar 232px, shell-bar ขาว, Placement Contract #46, Stepper #47, microcopy กลาง
...
> - ⭐ **06_TESTS microcopy-aware** — expected text อ้าง microcopy กลาง (toast/ปุ่ม) ของ html-generator-v4
> - 🔗 Chain sync: BRD v2.1 (upstream) / html-generator-v4 + ai-testcase-md-generator + qa-friendly-html-generator (downstream)
...
RIF v2.1 → html-generator-v6 → [ux gate + coverage gate ผ่าน] → brd-generator-full v2.2
...
| 4 | `/mnt/skills/user/html-generator-v6/SKILL.md` + `knowledge/iron-rules.md` + `knowledge/ci-tokens.md` + `knowledge/microcopy.md` | Layout Library (Pattern A-N) + Iron Rules + CI + microcopy (Sync Read — authoritative) — ถ้า v6 ไม่มี ให้หา `html-generator-v*` เวอร์ชันสูงสุดที่ติดตั้ง แล้ว log ว่าใช้ตัวไหน |
...
| BRD มี §14.6 Layout Deviation Log (legacy ก่อน 2026-07) | ✅ รับได้ — ใช้ Layout ID เดิมเป็น starting point แล้ว re-validate กับ html-generator-v6 ปัจจุบัน |
...
> FRD ห้ามขัด iron rules / CI ของ html-generator-v6 — ขัดเมื่อไหร่ standard ชนะ + log deviation
...
3. view /mnt/skills/user/html-generator-v6/SKILL.md     (Design Authority Sync Read ⭐)
   + view /mnt/skills/user/html-generator-v6/knowledge/iron-rules.md
   + view /mnt/skills/user/html-generator-v6/knowledge/ci-tokens.md
...
```
> **ห้าม hardcode pattern ใน skill นี้** — ความหมาย A-N อ่านสดจาก html-generator-v6 ทุก session
...
**1.5.2 กติกาตัดสิน:**
- **CI Lock:** palette/tokens = **ตามที่ html-generator-v6 ประกาศใน Sync Read เท่านั้น** (ปัจจุบัน Warm Light — แต่ห้ามจำค่า ให้อ่านสด) — BRD/RIF ขอสีนอก token → ปฏิเสธ + Open Question
...
- **06_TESTS:** (1) expected text: **มี HTML → ยึดข้อความจริงบนจอ (verbatim) เป็นอันดับแรก** แล้วค่อย fallback microcopy กลาง v6 (`html-generator-v6/knowledge/microcopy.md`) — ห้ามเดาคำเอง (2) Cross-Module Test Cases (§6.9) จาก §12.1 ("ยกเลิก PR → budget release จริง")
...
🎨 HTML มีอยู่แล้วในเลน (HTML-first) — ถ้าต้อง regen/patch ใช้ html-generator-v6 (อ่าน 01_UI.md — Layout Decision Log §1.0 คือคำสั่ง)
...
- ห้ามขัด iron rules / CI ของ html-generator-v6 (Sync Read) — standard ชนะ + log Functions Cut
...
| Sync Read | html-generator-25 (ตายแล้ว) | **html-generator-v4 (A-M, Drawer v2, #46-48, microcopy)** |
