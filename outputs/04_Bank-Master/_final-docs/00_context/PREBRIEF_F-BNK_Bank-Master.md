...
> Reverse Mode: HTML คือของจริง · **OQ-BNK-01..05 เคาะครบ (Strike 2026-08-11)** · ref GL Posting Group (F-PG DevPack, html+frd) · พร้อม gen FRD
...
| OB-3 | **Soft-ref → GL Posting Group (F-PG/0.19 kind=bank)**: บัญชี GL เงินฝาก resolve ผ่านกลุ่ม — **ไม่เก็บเลข COA ตรง** (มาตรฐาน BC Bank Posting Group) · mock 2 กลุ่ม (KBANK=active/SCB=draft) · จริง `GET /api/v1/gl/posting-groups?kind=bank&status=active` (generic F-PG-API-01 — ไม่มี endpoint เฉพาะ) · picker เห็นเฉพาะ active (F-PG BR-06) · binding optional (มติ BNK-02) | 0.19 + BC | §3, §7, OQ-BNK-02 |
...
| S-04 | Alt | แก้บัญชีที่มีเอกสารผ่านแล้ว | **IR-BNK-01 (CONFIRMED · Strike 2026-08-11)**: used>0 → ล็อก **ธนาคาร + เลขที่บัญชี** (UI disabled + lock-tag + logic guard — เปลี่ยนเลข = เปิดบัญชีใหม่แทน) · แก้ได้: ชื่อบัญชี/สาขา/ประเภท/พร้อมเพย์/กลุ่ม GL/ฝั่ง/★/สถานะ | — | เอกสารเก่า (snapshot) ไม่หลุด |
...
| BR-02 | **IR-BNK-01 (CONFIRMED · มติ 2026-08-11)**: used>0 ล็อกธนาคาร+เลขบัญชี — แก้ได้ที่เหลือ · ลบไม่ได้ (bulk ข้าม) · used = count Receipt/PV | S-04, S-07 | OQ-BNK-03 |
...
| ทิศ | คู่ | ผ่านอะไร |
|---|---|---|
| เข้า | ← **GL Posting Group (F-PG/0.19 kind=bank)** | dropdown กลุ่มบัญชี GL — `GET /api/v1/gl/posting-groups?kind=bank&status=active` (generic API-01) · เฉพาะ active (F-PG BR-06) |
| ~~ออก~~ | → ~~**Payment Method (0.20)**~~ **ตัดทิ้ง** | **OQ-BNK-01 = ไม่เอา (มติ Strike 2026-08-11)**: PM คง default_acct = COA ตรง — Bank Master ไม่ต้อง expose ให้ PM · OQ-PM-02 ไม่เกี่ยว Bank Master แล้ว |
...
|---|---|---|---|
| OQ-BNK-01 | PM (0.20) ควรชี้บัญชีจากทะเบียนนี้แทน COA ตรง | Strike | ✅ **CLOSED (ไม่เอา) — Strike 2026-08-11**: PM คง COA ตรง · ตัดเส้น → PM ทิ้ง |
| OQ-BNK-02 | บังคับผูกกลุ่ม GL ก่อนเปิดใช้มั้ย? · SCB=ร่างใน 0.19 | Strike | ✅ **RESOLVED — Strike 2026-08-11**: binding **optional** (บันทึกได้ เตือนเฉย ๆ) · picker เฉพาะ active (F-PG BR-06) |
| OQ-BNK-03 | IR-BNK-01: used>0 ล็อกธนาคาร+เลขบัญชี + นิยาม used | Strike | ✅ **CONFIRMED — Strike 2026-08-11**: ปลด [AI-DRAFT] · used = count เอกสาร Receipt/PV ผ่านบัญชี |
| OQ-BNK-04 | Receipt/PV — ยืนยัน flow เลือกบัญชี + snapshot + ★ pre-select | Strike | ✅ **CONFIRMED — Strike 2026-08-11**: active+ฝั่งตรง · ★ pre-select · snapshot ตอนบันทึก |
| OQ-BNK-05 | ยังไม่มี FRD (feature gen ใหม่) | BA | 🟢 **UNBLOCKED** — OQ ทั้งชุดเคาะแล้ว พร้อม gen FRD |
