# -*- coding: utf-8 -*-
"""
2-ROUND end-to-end verification suite for WarehouseBin.html (F-LOCATION-MASTER-001)
คลังและตำแหน่ง (Warehouse & Bin) — 5-level master, single-file SPA, hash routing, dual-view.

Two full passes; each runs on every viewport. A round PASSES only with 0 console + 0 page errors:

  Round 1  Happy-path lifecycle + Dual-view
           Build a whole warehouse: WH (branch + geo cascade) -> Zone (temp range) -> Area (allows_direct)
           -> Rack (grid) -> Location (2-step wizard, PACK, cap>0, rotation, flags, barcode)
           -> Bulk Generate (preview -> atomic commit). View location (3 tabs) + view node (fields/full_path/
           child counts). 5-level drill-down, KPI counts, branch in full_path. THEN the Hierarchy Tree view:
           List<->Hierarchy switcher, expand/collapse, tree select -> summary panel, child chip -> List drill,
           shared selection state preserved BOTH directions.

  Round 2  Negative / guards / RBAC / State / robustness (all must behave correctly)
           XOR parent, allows_direct block+suggest-rack, duplicate code EVERY level, capacity<=0, temp range
           missing, bulk collision atomic rollback, bulk grid=0 block, reparent-with-stock block (GAP-02),
           archive node with active children block + stays active (GAP-01), decommission-with-stock block,
           barcode-before-activate. RBAC operator/supervisor/manager + D13 role-switcher alignment.
           State: 7-state transitions + WORM audit, node soft-archive keeps record + 'จัดเก็บแล้ว' badge,
           reload mid-drill & mid-drawer, full Esc chain (combobox->modal->drawer), in-drawer modal z-index,
           drawer re-render after in-drawer tab switch, empty/loading/error states, geo cascade reset.

Together the two rounds cover everything the prior 3-round suite covered.
VERIFY ONLY — does not modify the HTML. Run with the project venv python.
"""
...
# =====================================================================
# ROUND 1 — happy-path lifecycle (build a whole warehouse) + dual-view
# =====================================================================
def round1(page, R, rnd, vw):
    sh = lambda n: shot(page, rnd, vw, n)

    # 0. root list renders + display name (D14)
    ...
    title = page.title()
    R.rec("D14 title shows คลังและตำแหน่ง (Warehouse & Bin)",
    ...
    R.rec("D14 sidebar label = คลังและตำแหน่ง", sb == "คลังและตำแหน่ง", sb)
    ...
    # 2. Zone (temp_controlled -> range) under new WH; branch chip in breadcrumb
    ...
    # =========================================================
    # DUAL-VIEW (D12) — List <-> Hierarchy tree
    # =========================================================
    # 10. view switcher toggles List <-> Hierarchy
    ...
    page.wait_for_timeout(150); sh("13_tree_expand_collapse")
    R.rec("D12 tree expand/collapse toggles children",
          exp['afterOpen'] > exp['before'] and exp['afterClose'] == exp['before'], json.dumps(exp))

    # 12. tree select -> summary panel (icon/level badge, code/status, full path, attribute fields, child chips)
    selinfo = page.evaluate("""() => {
        ...
        const badge = ns ? !!ns.querySelector('.ns-level, .level-badge, .pill') : false;
        ...
        return {zid:z.id, zcode:z.code, selLevel:state.sel.level, selId:state.sel.id,
                title, badge, pathOk: path.indexOf('›')>=0 && path.indexOf(z.code)>=0, chips};
    }""")
    page.wait_for_timeout(150); sh("14_tree_node_summary")
    R.rec("D12 tree select -> summary panel (sel synced + code + level badge + full path + child chips)",
          selinfo['selLevel'] == 'zone' and selinfo['selId'] == selinfo['zid']
          and selinfo['zcode'] in selinfo['title'] and selinfo['badge']
          and selinfo['pathOk'] and selinfo['chips'] >= 1,
          json.dumps(selinfo, ensure_ascii=False))

    # 13. child stat chip -> switches to List drilled to that node
    chip = page.evaluate("""() => {
    ...
    page.wait_for_timeout(200); sh("15_tree_chip_to_list")
    R.rec("D12 child chip -> switches to List drilled to that node",
          chip['view'] == 'list' and chip['level'] == 'zone', json.dumps(chip))

    # 14. shared selection preserved BOTH directions (list drill <-> tree select)
    ...
    page.wait_for_timeout(150); sh("16_shared_sel_bidir")
    R.rec("D12 shared selection preserved BOTH directions (List<->Tree)",
    ...
    page.evaluate("() => { state.view='list'; resetDrill(); state.treeOpen={}; render(); }")
    page.wait_for_timeout(150)


# =====================================================================
# ROUND 2 — negative / guards / RBAC / state / robustness
# =====================================================================
def round2(page, R, rnd, vw):
    ...
    xor_one = page.evaluate("() => state.form.rack_id==='R-A01' && !state.form.area_id")
    page.evaluate("setParentType('area')"); page.wait_for_timeout(200)
    xor_swap = page.evaluate("() => state.form.parent_type==='area' && !state.form.rack_id && !state.form.area_id")
    ...
    # 2. allows_direct=false -> block + 'เพิ่มชั้นวางก่อน' suggestion (A-AISLE-A allows_direct=false)
    ...
    def dup_node(level, drill_setup, code, expect_msg):
        ...
        page.evaluate("closeDrawer()"); page.wait_for_timeout(400)
    ...
    page.evaluate("() => { state.form.parent_type='rack'; state.form.rack_id='R-B01'; }")
    ...
    # Manager = full (create + del/archive)
    mgr = page.evaluate("() => { setRole('manager'); return perms(); }"); page.wait_for_timeout(200)
    R.rec("RBAC Manager: full (create + del/archive + status)",
          mgr["create"] and mgr["del"] and mgr["status"], json.dumps(mgr))
    # D13 role switcher aligned with create button
    align = page.evaluate("""() => {
    ...
    sh("13_role_align")
    R.rec("D13 role switcher aligned with create button (top & height within 2px)",
          bool(align) and align['dTop'] <= 2 and align['dH'] <= 2, json.dumps(align))

    # ---- STATE / ROBUSTNESS ----
    # 12. Location 7-state transitions + WORM audit  (L005 PACK-01: active, has barcode, empty)
    ...
    # 13. node soft-archive keeps record + 'จัดเก็บแล้ว' badge (WH-03 childless)
    ...
    # 14. hash-routing refresh-safe — reload mid-drill
    ...
    # in-drawer modal z-index + Esc tier 2/3
    ...
    sh("20_modal_over_drawer_zindex")
    try:
    ...
    # 16. drawer content re-render after in-drawer tab switch (content persists, no blank)
    ...
    # 17. empty state (no-match filter at root)
    ...
    # 19. error state — drill into a deleted/missing node
    ...
    # 20. geo cascade reset when province changes
...
def main():
    ...
    out = QC / "E2E_2_ROUNDS_RESULT.json"
