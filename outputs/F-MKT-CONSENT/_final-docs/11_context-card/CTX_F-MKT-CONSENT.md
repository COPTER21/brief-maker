...
> **derived from:** FRD_F-MKT-CONSENT v1.0 (2026-09-13) · **generated:** 2026-09-13
> **module:** การตลาด (Marketing · PDPA/Policy Center scope) · **feature code:** F-MKT-CONSENT (plan id F058) · **wave:** W1 · **variant:** FULL · **status:** active (FRD DRAFT · awaiting BA/PM gate 2)
...
| `T_consent_request` | id | tenant_id, code `REQ-xxxx`, `subject_ref` (Confidential+PII), channel, status, `link_token` (Confidential), `ref_old` (FK renew/reconsent), expires_at (created+30วัน) | header คำขอ |
| `T_consent_request_purpose` | id | request_id (FK), purpose_id (FK) | join · UNIQUE(request,purpose) |
| `T_consent_request_send` | id | request_id (FK), channel, sent_at, kind | **append-only** (การส่ง/ส่งซ้ำ · BR-18) |
...
| `T_consent_evidence` | id | consent_id (FK 1:1), answered_at, request_channel, `id_method` (Confidential+PII), policy_version, `ip_device` (Confidential+PII) | **หลักฐาน 5 อย่าง** (BR-15) · NULL ได้เฉพาะ declined/pending |
...
| `request.status` | `draft` → `pending` (ส่งครั้งแรก) → `answered` · หรือ `pending` → `expired` (เกิน 30 วัน = lapsed ≠ declined) | feature นี้ |
...
### State transitions (owner = feature นี้ · จาก 05_RULES §5.2)
...
| granted | withdrawn | ถอน (มีผลทันที · ไม่อนุมัติ) | officer/dpo | reason + via required |
...
- `T_consent_request` 1—N `T_consent_request_send` (append-only) · 1—N `T_consent` (split per-purpose ตอน answer)
...
> Source: FRD 02_API. ทุก endpoint ต้อง `X-Tenant-Id` (RLS) · ทุก mutation รับ `Idempotency-Key` header · ไม่มี PUT/PATCH (ใช้ POST action ตาม state machine). recipient endpoints (API-12/13) เข้าผ่าน **token** (public link · ไม่ต้อง JWT).
...
| POST | `/consent/requests/:id/send` | ส่ง/ส่งซ้ำ (**mock** · ส่งซ้ำ = send record ครั้งที่ N) | FN-06 |
...
| GET | `/consent/recipient/:token` | โหลด recipient view (public via token · lapsed → 200 `{expired:true}` ไม่ 404) | FN-08 |
| POST | `/consent/recipient/:token/answer` | ส่งคำตอบยินยอม/ปฏิเสธ **รายข้อ** (verified=true) → registry + evidence 5 | FN-09/10/11 |
...
| POST | `/consent/registry/:id/withdraw` | ถอน (มีผลทันที · reason + via required) | FN-13 |
...
| BR-19 | `/consent/resolve` ตอบ **HTTP 200 เสมอ** · never_asked → `allowed:false` (ไม่ 404) | แคมเปญ/ส่งข้อความ · F143 · gateway |
...
| BR-21 | ผู้เรียกแคชผล resolve **≤5 นาที + ล้างทันทีเมื่อถอน** (caller-side contract · no UI · OQ-02) | ทีม integration / แคมเปญ |
...
| BR-CSQ-02 | envelope มี `idempotency_key` unique ต่อ (feature, ref, action) | 7C Engine |
...
**Permission (business intent · HTML `PERM()` · enforce จริง = OQ-05):** officer = ดูทะเบียน/สร้าง-ส่งคำขอ/ถอน/resolve (❌ purpose/close) · dpo = ทุกอย่าง (+ สร้าง/แก้ purpose, upload version, close) · auditor = read + resolve(read) เท่านั้น. **ไม่มี approval chain** (LOCK-04) — การควบคุมอยู่ที่ default-deny + evidence 5 + audit append-only.
...
- **Depended by (downstream):**
  - **แคมเปญส่งข้อความ / จดหมายข่าว** — เรียก `POST /consent/resolve` (ก่อนส่งทุกครั้ง · allowed + status + reason)
  ...
  - **DSAR (F157) / consent receipt** — อ่านประวัติ consent (T_consent_history) + Customer-360 (E1) เป็นแหล่งหลักฐาน — **deferred (E3 · wave DSAR)** [ตาม PROPOSALS_outbound §; consent receipt PDF ยังไม่อยู่ใน scope รอบนี้]
- **Declarations (รอบนี้ 2026-09-13):**
  - **CSQ** [yes · declare-only] — 7 events เข้าท่อ **SecC** `[DEFAULT — รอยืนยัน · OQ-CSQ-02]` (PII/permission change) · ไม่ประกาศ OC/DC/SC/EC/AC/FC · profile_id `CSQ-F058` `[DEFAULT · OQ-CSQ-01]` · sink = `T_consent_csq_outbox` → deploy pipeline ส่งเข้า 7C
...
- **Open dependencies (blocking):**
  - **OQ-03** (YES · ก่อน implement) — วิธียืนยันตัวตนจริงใน recipient view (ปัจจุบัน checkbox **mock** → ลิงก์/OTP/สแกน) · กระทบ `evidence.id_method` — **ห้าม dev เดา**
  ...
  - (non-blocking) OQ-02 caller cache contract · OQ-05 permission enforce จริงต่อ role (แทน persona switcher เดโม) + consent receipt PDF (E3) deferred · OQ-07 file validation จริง · OQ-08 timezone `expires_at` · OQ-CSQ-01/02 profile_id + ชื่อท่อ SecC

---
*trace: §1 ← FRD 00_OVERVIEW §0.3 + BRD §12.1 · §2 ← FRD 04_DB · §3 ← FRD 02_API (+03_LOGIC §3.4 events) · §4 ← FRD 05_RULES §5.1 · §5 ← FRD 00_OVERVIEW §0.5 + BRD §12.1 + CSQ_BRIEF + PROPOSALS_outbound*
